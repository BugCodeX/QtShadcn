...
class TestThemeConfig:
    """Tests for the theme config model."""

    def test_default_config_has_no_source_path(self):
        """Test that the default config has no source path."""
        config = ThemeConfig()
        assert config.theme_source_path is None
        assert config.theme_mode == "auto"

    def test_config_stores_source_path(self):
        """Test that the config stores the source path."""
        config = ThemeConfig(theme_source_path="/tmp/theme.xml")
        assert config.theme_source_path == "/tmp/theme.xml"
